...
def normalityCheck(flow_data, repoPath, uf):
    ...
    ax[0].hist(np.log(aqData['flow'].dropna()), facecolor='light blue')
    ax[0].set_title('Normal (Log)')
    ...
    if (flow_data > 0).all():
        ...
        ax[1].hist(transformed_data, facecolor='purple')
        ax[1].set_title('BoxCox')
        ax[1].set_xlabel('Vazão (Box-Cox)')
        ax[1].set_ylabel('Frequência')
    else:
        ax[1].hist(flow_data, facecolor='purple')
        ax[1].set_title('BoxCox (Dados não podem ser transformados)')
        ax[1].set_xlabel('Vazão')
        ax[1].set_ylabel('Frequência')

    # Dados originais
    ax[2].hist(flow_data, facecolor='pink')
    ax[2].set_title('Hidrograma Original')
    ...
    fig.savefig(repoPath + '/figuras/' + uf + '/histogramDataNormalizationBica_' + 'flow' + '.png')
